...
- **驗證/實測/跑起來確認/宣告完成前** → `verifying-changes` (Runtime evidence, not just tests)
- **Code Review/找 bug/審 diff/PR 審查** → `reviewing-code-changes` (Verified findings only)
- **重構/精簡/去重/清理** → `simplifying-code` (Quality only, behavior preserved)
- **圖表/視覺化/儀表板/報告配圖** → `visualizing-data` (Form + color system BEFORE chart code)
- **AGENTS.md/CLAUDE.md/專案上下文初始化** → `initializing-agent-context` (Verified commands, 100-line cap)
